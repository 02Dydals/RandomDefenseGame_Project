...
public class HeroEditor : Editor
{
    ...
    private void DrawParameterField(Rect rect, string label, SerializedProperty parameterProp, int index)
    {
        EditorGUI.LabelField(new Rect(rect.x, rect.y, 120, EditorGUIUtility.singleLineHeight), label);

        // 첫번째 필드
        parameterProp.GetArrayElementAtIndex(index).floatValue = 
